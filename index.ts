...
export default function piIntercomExtension(pi: ExtensionAPI) {
  ...
  const replyWaiterKey = (from: string, replyTo: string) => `${from.toLowerCase()}\u0000${replyTo}`;
  function waitForReply(from: string, replyTo: string, signal?: AbortSignal): Promise<Message> {
    const key = replyWaiterKey(from, replyTo);
    if (replyWaiters.has(key)) {
      return Promise.reject(new Error("Already waiting for this reply"));
    }
    if (signal?.aborted) {
  ...
  function rejectReplyWaiter(from: string, replyTo: string, error: Error): void {
    replyWaiters.get(replyWaiterKey(from, replyTo))?.reject(error);
  }
  ...
  function handleIncomingMessage(ctx: ExtensionContext, from: SessionInfo, message: Message): void {
    ...
    const possibleReplyKeys = [
      replyWaiterKey(senderTarget, message.replyTo ?? ""),
      replyWaiterKey(from.id, message.replyTo ?? ""),
    ];
    for (const key of possibleReplyKeys) {
      const waiter = replyWaiters.get(key);
      if (waiter) {
    ...
    pi.registerTool({
      ...
      async execute(_toolCallId, params, signal, _onUpdate, ctx) {
        ...
        try {
          ...
          if (signal?.aborted) {
            rejectReplyWaiter(sendTo, questionId, new Error("Cancelled"));
            try {
              await replyPromise;
            } catch {
          ...
          if (!sendResult.delivered) {
            ...
            rejectReplyWaiter(sendTo, questionId, new Error(`Message to "${metadata.orchestratorTarget}" was not delivered: ${errorText}`));
            if (replyPromise) {
              try {
                await replyPromise;
              } catch {
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      switch (action) {
        ...
        case "ask": {
          ...
          try {
            sendTo = await resolveSessionTarget(connectedClient, to) ?? to;
            if (_signal?.aborted) {
              return {
                content: [{ type: "text", text: "Cancelled" }],
                isError: true,
            ...
            if (!sendResult.delivered) {
              const errorText = sendResult.reason ?? "Session may not exist or has disconnected.";
              rejectReplyWaiter(sendTo, questionId, new Error(`Message to "${to}" was not delivered: ${errorText}`));
              if (replyPromise) {
                try {
                  await replyPromise;
                } catch {
